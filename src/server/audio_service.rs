...
pub fn set_voice_call_mixing(enabled: bool) {
    #[cfg(not(any(target_os = "linux", target_os = "android")))]
    {
        ...
        *is_active = enabled;
        ...
        // Clear AEC buffers when voice call state changes
        if !enabled {
            clear_client_audio_ref_buffer();
        }
...
/// Push decoded client audio samples to the reference buffer for AEC processing
/// The audio should already be at 48kHz stereo format
#[cfg(windows)]
pub fn push_client_audio_ref(samples: &[f32]) {
    #[cfg(not(any(target_os = "linux", target_os = "android")))]
    {
...
/// Clear the client audio reference buffer (called when voice call ends)
#[cfg(windows)]
pub fn clear_client_audio_ref_buffer() {
    #[cfg(not(any(target_os = "linux", target_os = "android")))]
    {
...
mod cpal_impl {
    ...
    fn mix_and_send(
        ...
        let speaker_resampled: Vec<f32> = speaker_resampled.iter().map(|s| s * volume_gain).collect();
